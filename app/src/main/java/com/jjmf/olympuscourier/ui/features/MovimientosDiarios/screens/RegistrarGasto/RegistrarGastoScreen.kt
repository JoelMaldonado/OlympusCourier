...
import com.jjmf.olympuscourier.util.alertaPrecio
import com.jjmf.olympuscourier.util.isNumeric
...
fun RegistrarGastoScreen(
    ...
    if (viewModel.alertaPrecio){
        alertaPrecio(context){
            viewModel.alertaPrecio = false
        }
    }
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        ...
        Column(
            ...
            CajaTexto(
                ...
                color = ColorR1,
                trailingIcon = {
                    if (viewModel.detalle.isNotEmpty()){
                        Icon(imageVector = Icons.Default.Check, contentDescription = null, tint = ColorR1)
                    }
                }
            )
            CajaTexto(
                ...
                color = ColorR1,
                mensajeError = if (viewModel.costo.isEmpty()) "Importe mínimo S/1" else null
            )
            Row(
                verticalAlignment = Alignment.CenterVertically
            ) {
            ...
            Row(
                ...
                Button(
                    onClick = {
                        if (viewModel.costo.isNumeric()){
                            if (viewModel.costo.toDouble() > 1) viewModel.insert(context)
                            else viewModel.alertaPrecio = true
                        }else viewModel.alertaPrecio = true
                    },
                    modifier = Modifier.weight(1f),
                    colors = ButtonDefaults.buttonColors(
                    ...
                    enabled = !viewModel.progress && viewModel.detalle.isNotEmpty() && viewModel.costo.isNotEmpty() && viewModel.imagen != null
                ) {
